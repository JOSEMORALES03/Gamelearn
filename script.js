...
function guardarResultadosEnLocalStorage(puntuacion, total, tiempo, tipoJuego, errores) {
    ...
    const nuevoResultado = {
        ...
        grado: gradoActual,
        grupo: grupoActual, // Guardamos el grupo
...
async function mostrarPanelDocente() {
    ...
    // --- Lógica para Panel de Visualizaciones ---
    const panelVisualizaciones = document.getElementById('panel-visualizaciones-derecha');
    ...
    // NUEVO: Referencias a los filtros de visualización
    ...
    Array.from({ length: 5 }, (_, i) => i + 1).forEach(g => {
        filtroGrupo.innerHTML += `<option value="${g}">Grupo ${g}</option>`;
    });

    // NUEVO: Poblar filtros de visualización
    function poblarFiltrosVis(datos) {
        ...
        const juegosUnicos = [...new Set(datos.map(d => d.tipoJuego))];

        filtroGradoVis.innerHTML = '<option value="">Todos</option>' + gradosUnicos.map(g => `<option value="${g}">${g}°</option>`).join('');
        filtroGrupoVis.innerHTML = '<option value="">Todos</option>' + gruposUnicos.map(g => `<option value="${g}">${g}</option>`).join('');
        filtroJuegoVis.innerHTML = '<option value="">Todos</option>' + juegosUnicos.map(j => `<option value="${j}">${JUEGOS_DISPONIBLES[j]?.nombre || j}</option>`).join('');
    }

    // Event listeners para los filtros de visualización
    [filtroGradoVis, filtroGrupoVis, filtroJuegoVis].forEach(filtro => {
        filtro.addEventListener('change', () => {
            // Re-aplicamos los filtros principales para que todo se actualice
            aplicarFiltros();
        });
    });
    ...
    function aplicarFiltros() {
        ...
        if (grupoSeleccionado) {
            resultadosFiltrados = resultadosFiltrados.filter(res => res.grupo === grupoSeleccionado);
        }

        renderTabla(resultadosFiltrados);
        // NUEVO: Actualizar las visualizaciones con los datos filtrados
    ...
    function actualizarVisualizaciones(datos) {
        ...
        poblarFiltrosVis(resultados); // Usamos los resultados originales para tener todas las opciones
        ...
        const gradoVis = filtroGradoVis.value;
        const grupoVis = filtroGrupoVis.value;
        const juegoVis = filtroJuegoVis.value;

        let datosVis = datos;
        if (gradoVis) datosVis = datosVis.filter(d => d.grado === gradoVis);
        if (grupoVis) datosVis = datosVis.filter(d => d.grupo === grupoVis);
