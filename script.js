...
async function iniciarApp(grado, grupo) {
    ...
    gradoActual = grado;
    grupoActual = grupo;
    gradoSeleccionadoEl.textContent = `${grado}° Grado - Grupo ${grupo}`;
...
document.addEventListener('DOMContentLoaded', () => {
    // Iniciar la animación del splash screen al cargar la página
    showSplashScreen();
...
async function mostrarPanelDocente() {
    ...
    // --- Poblar filtros ---
    function poblarFiltrosPrincipales(datos) {
        const materiasUnicas = [...new Set(datos.map(d => d.materia))].sort();
        const juegosUnicos = [...new Set(datos.map(d => d.tipojuego))].filter(Boolean); // Filtra nulos/undefined
        const gradosUnicos = [...new Set(datos.map(d => d.grado))].sort((a, b) => a - b);
        const gruposUnicos = [...new Set(datos.map(d => d.grupo))].filter(Boolean).sort((a, b) => a - b);
        ...
        juegosUnicos.forEach(clave => {
            const juego = JUEGOS_DISPONIBLES[clave];
            if (juego) filtroJuego.innerHTML += `<option value="${clave}">${juego.nombre}</option>`;
        });
        ...
        gruposUnicos.forEach(g => {
            filtroGrupo.innerHTML += `<option value="${g}">Grupo ${g}</option>`;
        });
    }

    function poblarFiltrosVis(datos) {
        const materiasUnicas = [...new Set(datos.map(d => d.materia))];
        const gradosUnicos = [...new Set(datos.map(d => d.grado))];
        const gruposUnicos = [...new Set(datos.map(d => d.grupo))];
        const juegosUnicos = [...new Set(datos.map(d => d.tipojuego))].filter(Boolean);

        filtroMateriaVis.innerHTML = '<option value="">Todas</option>' + materiasUnicas.map(m => `<option value="${m}">${m}</option>`).join('');
        filtroGradoVis.innerHTML = '<option value="">Todos</option>' + gradosUnicos.map(g => `<option value="${g}">${g}°</option>`).join('');
        filtroGrupoVis.innerHTML = '<option value="">Todos</option>' + gruposUnicos.map(g => `<option value="${g}">Grupo ${g}</option>`).join('');
        filtroJuegoVis.innerHTML = '<option value="">Todos</option>' + juegosUnicos.map(j => {
            const nombreJuego = JUEGOS_DISPONIBLES[j]?.nombre || j;
            return `<option value="${j}">${nombreJuego}</option>`;
        }).join('');
    }
    ...
    [filtroMateriaVis, filtroGradoVis, filtroGrupoVis, filtroJuegoVis].forEach(filtro => {
        filtro.onchange = aplicarFiltros;
    });
        ...
        for (const nombreEstudiante of estudiantesOrdenados) {
            ...
            tablaHtml += `
                <div class="nombre-estudiante-header" style="margin-top: 20px;">
                    <h3 class="nombre-estudiante-tabla">${nombreEstudiante} - ${gradoEstudiante}° Grado (Grupo ${grupoEstudiante})</h3>
    ...
    btnLimpiar.addEventListener('click', () => {
        ...
        filtroGrado.value = '';
        filtroGrupo.value = '';
        poblarFiltrosPrincipales(resultados); // Repoblar con todas las opciones
    ...
    document.getElementById('btn-exportar-excel').addEventListener('click', () => {
        ...
        if (!nombreEstudiante) {
            alert("Para exportar, primero debes buscar y filtrar los resultados de un estudiante específico por su nombre.");
            return;
        }
        // La función exportarAExcel ya comprueba si hay datos para exportar.
    ...
    function actualizarVisualizaciones(datos) {
        ...
        // Guardar la selección actual de los filtros de visualización
        const materiaVisPrevia = filtroMateriaVis.value;
        const gradoVisPrevio = filtroGradoVis.value;
        const grupoVisPrevio = filtroGrupoVis.value;
        const juegoVisPrevio = filtroJuegoVis.value;
        
        // Poblar los filtros de visualización con los datos actuales
        poblarFiltrosVis(datos);

        // Restaurar la selección previa
        filtroMateriaVis.value = materiaVisPrevia || '';
        filtroGradoVis.value = gradoVisPrevio || '';
        filtroGrupoVis.value = grupoVisPrevio || '';
        filtroJuegoVis.value = juegoVisPrevio || '';
    ...
    // Render inicial
    poblarFiltrosPrincipales(resultados);
 ...
 */
 function exportarAExcel(datos) {
     ...
     const rows = datos.map(res => [
         ...
         formatCSVField(res.puntuacion_str),
