...
async function mostrarPanelDocente() {
    ...
    function repoblarFiltrosVis(datosBase) {
        ...
        // Obtener opciones únicas de TODOS los datos (sin cascada, para que
        // las opciones nunca se vacíen sin importar lo que esté seleccionado)
        const materiasUnicas = [...new Set(datosBase.map(d => d.materia))].filter(Boolean).sort();
        const gradosUnicos = [...new Set(datosBase.map(d => String(d.grado)))].filter(g => g && g !== 'null').sort((a, b) => a - b);
        const gruposUnicos = [...new Set(datosBase.map(d => String(d.grupo)))].filter(g => g && g !== 'null' && g !== 'undefined').sort((a, b) => a - b);
        const juegosUnicos = [...new Set(datosBase.map(d => d.tipojuego))].filter(Boolean);
    ...
    function repoblarFiltros(datos) {
        ...
        // Obtener opciones únicas de TODOS los datos (sin cascada, para que
        // las opciones nunca se vacíen sin importar lo que esté seleccionado)
        const materiasUnicas = [...new Set(datos.map(d => d.materia))].filter(Boolean).sort();
        const juegosUnicos = [...new Set(datos.map(d => d.tipojuego))].filter(Boolean);
        const gradosUnicos = [...new Set(datos.map(d => String(d.grado)))].filter(Boolean).sort((a, b) => a - b);
        const gruposUnicos = [...new Set(datos.map(d => String(d.grupo)))].filter(g => g && g !== 'null' && g !== 'undefined').sort((a, b) => a - b);
